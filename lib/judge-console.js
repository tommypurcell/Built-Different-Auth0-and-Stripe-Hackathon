// Same-origin proxy (see app/api/judge/[...path]/route.js). Requests are
// forwarded server-side to the real upstream, avoiding browser CORS blocks.
const DEFAULT_BASE_URL = "/api/judge";
...
export function buildSubmissionSnapshot(submission) {
  // The upstream API requires a non-empty test_instructions. Submissions with
  // no instructions fall back to a generic prompt so the batch validates.
  const instructions = submission.instructions?.trim();
  return {
    submission_id: submission.id,
    product_url: submission.productUrl,
    test_instructions: instructions || "test this",
  };
}
